...
from keras.layers import Flatten, Dense, Input, Conv1D, MaxPooling1D, GlobalAveragePooling1D, GlobalMaxPooling1D, AveragePooling1D, LeakyReLU
...
### CNN Best model
def cnn_best(input_shape=(700,1), classes=256):
    # From VGG16 design
...
def cnn_best_nosoftmax(input_shape=(700,1), classes=256):
    # From VGG16 design
    img_input = Input(shape=input_shape)
    # Block 1
    x = Conv1D(64, 11, activation='relu', padding='same', name='block1_conv1')(img_input)
...
def train_model(X_profiling, Y_profiling, model, save_file_name, epochs=150, batch_size=100, validation_data=None):
    X_attack, Y_attack = validation_data
    ...
    history = model.fit(x=Reshaped_X_profiling, y=to_categorical(Y_profiling, num_classes=256), batch_size=batch_size, verbose = 1, epochs=epochs, callbacks=callbacks, validation_data=(Reshaped_X_attack, to_categorical(Y_attack, num_classes=256)))
    pickle.dump(history, open("ASCAD_history.p", "wb"))
    return history
...
if __name__ == "__main__":
    ...
    train_model(X_profiling, Y_profiling, cnn_best_model, ascad_trained_models_folder + "my_cnn_best_desync0_epochs75_batchsize200.h5", epochs=75, batch_size=200, validation_data=(X_attack, Y_attack))
    #### Desync = 50
    #cnn_best_model = cnn_best()
